...
write.csv(summary_stats, file.path(opt$output, "summary_stats.csv"), row.names = FALSE)
...
ggplot(results, aes(x = logFC, y = -log10(PValue), color = significance)) +
    ...
    labs(title = paste("Volcano Plot -", opt$compare), 
         x = "Log2 Fold Change", y = "-Log10 P-value",
         color = "Methylation Status") +
    theme_minimal() +
    theme(legend.position = "right")
ggsave(file.path(opt$output, "volcano_plot.png"), width = 10, height = 8)
...
ggplot(results, aes(x = logCPM, y = logFC, color = significance)) +
    ...
    labs(title = paste("MA Plot -", opt$compare), 
         x = "Log2 CPM", y = "Log2 Fold Change",
         color = "Methylation Status") +
    theme_minimal() +
    theme(legend.position = "right")
ggsave(file.path(opt$output, "ma_plot.png"), width = 10, height = 8)

# Print the cutoffs and colors used
cat(sprintf("Analysis performed with:\nlogFC cutoff: %f\np-value cutoff: %f\n", 
            opt$logfc_cutoff, opt$pvalue_cutoff))
